...
%    within.
    ...
    global FX_Zero FY_Zero FZ_Zero TAUX_Zero TAUY_Zero TAUZ_Zero;
    if nt == 1
        FX_Zero = FX;
        FY_Zero = FY;
        TAUZ_Zero = TAUZ;
    ...
    for index = 1:15
        FX(index)=FX_Zero(index);
        FY(index)=FY_Zero(index);
        TAUZ(index)=TAUZ_Zero(index);
    end
    for index = 31:45
        FX(index)=FX_Zero(index);
        FY(index)=FY_Zero(index);
    ...
    for index = 1:15
        FZ(index)=FZ_Zero(index);
        TAUX(index)=TAUX_Zero(index);
        TAUY(index)=TAUY_Zero(index);
    end
    for index = 31:45
        FZ(index)=FZ_Zero(index);
        TAUX(index)=TAUX_Zero(index);
