...
for sep = smallest_sep: increment: largest_sep
    ...
    %  orientation of THETA=0 but you can change this here.)
        ...
        J0invERROR_VECk1 = zeros(3*tot_seg,1);  % J_0^{-1} f(X_(k+1))  in Algorithm 2
           ...
           TOTAL_STEPS_TO_RUN = TOTAL_STEPS*2/3;
           for nt = 1:TOTAL_STEPS_TO_RUN
            ...
            if nt == TOTAL_STEPS_TO_RUN
                ...
                velocity_output(2, index) = sqrt(abs(body_velocity_X)^2+abs(body_velocity_Y)^2)/0.0296706588967878;
            end
...
title('Maximum Velocity of Three Swimmers In Phase')
text(1.0553, 1.00526647220635,'   1.005266')
velocity_max = y(1);
for i = 2:length(y);
    if y(i) > velocity_max;
        seperation_max = x(i)
