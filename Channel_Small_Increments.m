...
Output_Index = 0;
spacings = [0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7];
Spacing_And_Velocity = zeros(2,length(spacings));
...
for v_s = spacings
    ...
    %  orientation of THETA=0 but you can change this here.)
        ...
        J0invERROR_VECk1 = zeros(3*tot_seg,1);  % J_0^{-1} f(X_(k+1))  in Algorithm 2
           ...
           for nt = 1:TOTAL_STEPS/5
            ...
            if(nt == 1)
                ...
                X_Zero = X_T;
                Y_Zero = Y_T;
                THETA_S_Zero = THETA_T;
                TX_S_Zero = cos(THETA_S_Zero);
                TY_S_Zero = sin(THETA_S_Zero);
                X_S = X;
                Y_S = Y;
                THETA_S = THETA;
                TX_S = cos(THETA_S);
            ...
            % Reset initial values for worms 1 and 3
            for index = 1:15
                X_S(index) = X_Zero(index);
                X_S(index+30) = X_Zero(index+30);
                Y_S(index) = Y_Zero(index);
                Y_S(index+30) = Y_Zero(index+30);
                THETA_S(index) = THETA_S_Zero(index);
                THETA_S(index+30) = THETA_S_Zero(index+30);
                TX_S(index) = TX_S_Zero(index);
                TX_S(index+30) = TX_S_Zero(index+30);
                TY_S(index) = TY_S_Zero(index);
                TY_S(index+30) = TY_S_Zero(index+30);
            end
        ...
        Spacing_And_Velocity(1, Output_Index) = v_s;
        Spacing_And_Velocity(2, Output_Index) = abs(body_velocity_X);
end
