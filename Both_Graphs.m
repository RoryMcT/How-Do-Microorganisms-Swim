...
Num_worm = 3;                     % number of filaments
...
for sep = smallest_sep: increment: largest_sep
        ...
        J0invERROR_VECk1 = zeros(3*tot_seg,1);  % J_0^{-1} f(X_(k+1))  in Algorithm 2
           ...
           for nt = 1:TOTAL_STEPS/5
            ...
            if nt == 60
                ...
                velocity_output(2, index) = sqrt(abs(body_velocity_X)^2+abs(body_velocity_Y)^2);
            end
    ...
    %    within.
        ...
        [TAUZ] = intrinsic_torques(TAUZ, KB, K, Dist_seg_cent, omega, t + dt, T_0, SW_IND, PHASE);
...
%velocity_output
x = velocity_output(1,:)/0.0297;
y = velocity_output(2,:)/0.0297;
...
increment = 0.05;
size = ((largest_sep-smallest_sep)/increment)+1;
P_S_velocity_output = zeros(2, round(size));
for sep = smallest_sep: increment: largest_sep
    index = index + 1;
    % Set up segment position vectors.
    %   X_S is x^(j+1), i.e. at next timestep (which we are solving for)
    ...
    %  orientation of THETA=0 but you can change this here.)
        ...
        J0invERROR_VECk1 = zeros(3*tot_seg,1);  % J_0^{-1} f(X_(k+1))  in Algorithm 2
           ...
           for nt = 1:TOTAL_STEPS/5
            ...
            % If ||ERROR_VECk|| < concheck_tol (= epsilon in Alg 2, Line 4),
            % then concheck = 0. Else, 1.
            [concheck,ERROR_VECk,VY] = F2(X_S,Y_S,TX_S,TY_S,THETA_S,LAMBDA1,LAMBDA2,concheck_tol);
            % (VY only being outputted here for calculating effective drag later.)
            ...
            while(concheck == 1) % Alg 2, Line 4
                ...
                % Check to see if the new state is an acceptable solution:
                % ERROR_VECk1 = f(X_(k+1))
                [concheck, ERROR_VECk1,VY] = F2(X_S,Y_S,TX_S,TY_S,THETA_S,LAMBDA1,LAMBDA2,concheck_tol);
            ...
            if nt == 60
                P_S_velocity_output(1, index) = sep;
                P_S_velocity_output(2, index) = sqrt(abs(body_velocity_X)^2+abs(body_velocity_Y)^2);
            end
    ...
    function [concheck_local,ERROR_VECk1_local,VY] = F2(X_S, Y_S, TX_S, TY_S,...
                                                       THETA_S, LAMBDA1,...
                                                       LAMBDA2, tol)
    % F  places forces and torques on the segments, calculates the resultant
    %    velocities and angular velocities, and forms the error vector f(X*).
    ...
    %    within.
        ...
        [TAUZ] = P_S_intrinsic_torques(TAUZ, KB, K, Dist_seg_cent, omega, t + dt, T_0, SW_IND, PHASE);
...
velocity_output
P_S_velocity_output
x2 = P_S_velocity_output(1,:)/0.0297;
y2 = P_S_velocity_output(2,:)/0.0297;
plot(x,y,'x-','LineWidth',3)
hold on
plot(x2,y2,'x-','LineWidth',3)
xlabel('Separation')
ylabel('Velocity')
legend('Shift = 0', 'Shift = pi')
end
